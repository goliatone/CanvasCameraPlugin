           ...
           if (error) {
               ...
               dispatch_sync(queue, ^(void) {
                   ...
                   if(strData != nil){
                       ...
                       NSLog(@"image uri nil?...");
                       [dicRet setObject:@"" forKey:@"imageURI"];
                   }
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            @autoreleasepool {
                ...
                if ([self.webView respondsToSelector:@selector(stringByEvaluatingJavaScriptFromString:)]) {
                    // Cordova-iOS pre-4
                    [self.webView performSelectorOnMainThread:@selector(stringByEvaluatingJavaScriptFromString:) withObject:javascript waitUntilDone:NO];
                } else {
                    // Cordova-iOS 4+
